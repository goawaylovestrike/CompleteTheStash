...
from FansDbClient import FansDbClient
...
class CompleteTheStashConfiguration:
    ...
    FansDBSceneSource: MissingSceneSource
...
def get_complete_the_stash_config(local_configuration) -> CompleteTheStashConfiguration:
    ...
    if (
        ...
        and not complete_the_stash_config.get("missingFansDBAddress")
        and not complete_the_stash_config.get("missingFansDBApiKey")
    ...
    ) or complete_the_stash_config.get("missingFansDBApiKey"):
        if not complete_the_stash_config.get("missingFansDBAddress"):
            raise ValueError(
                "URL for missing Stash for FansDB scenes is not configured. Please set in Plugins configuration."
            )
        if not complete_the_stash_config.get("missingFansDBApiKey"):
            raise ValueError(
                "API Key for missing Stash for FansDB scenes is not configured. Please set in Plugins configuration."
            )
    ...
    FansDB_configuration = None
    if complete_the_stash_config.get(
        "missingFansDBAddress"
    ) and complete_the_stash_config.get("missingFansDBApiKey"):
        FansDB_configuration = MissingSceneSource(
            stashappAddress=complete_the_stash_config.get("missingFansDBAddress"),
            stashappApiKey=complete_the_stash_config.get("missingFansDBApiKey"),
            stashboxEndpoint="hhttps://fansdb.cc/graphql",
        )
    ...
    return CompleteTheStashConfiguration(
        ...
        FansDBSceneSource=FansDB_configuration,
...
def execute():
    ...
    # FansDB
    if complete_the_stash_config.fansDBSceneSource:
        missing_stash_client = create_missing_stash_client(
            complete_the_stash_config.stashDbSceneSource
        )
        missing_configuration = missing_stash_client.get_configuration()
        ...
        stashbox_config = get_matching_stashbox_config(
            local_configuration, complete_the_stash_config.FansDbSceneSource
        )
        stashbox_client = FansDBClient(
            stashbox_config["endpoint"],
            stashbox_config["api_key"],
        )
        ...
        config = {
            "performerTags": complete_the_stash_config.performerTags,
            "stashboxEndpoint": complete_the_stash_config.fansDBSceneSource.stashboxEndpoint,
            "sceneExcludeTags": complete_the_stash_config.sceneExcludeTags,
            "enableSceneHooks": complete_the_stash_config.enableSceneHooks,
        }
        stash_completer = StashCompleter(
